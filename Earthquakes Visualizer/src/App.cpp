...
void App::onEvent(shared_ptr<Event> event) {
	...
	// Rotate the earth when the user clicks and drags the mouse
        ...
        if (mouseDown){
            ...
            mat4 xroll = toMat4(angleAxis(radians(dxy.x * delta), vec3(0,1,0)));
            mat4 yroll = toMat4(angleAxis(radians(dxy.y * delta), vec3(1,0,0)));
            rotation = xroll * yroll * rotation;
        }
	}
}
...
void App::onRenderGraphics() {
	...
	for (int x=start; x<end; x++) {
		...
		// Draw earthquake e
        vec3 position = earth->getPosition(e.getLatitude(), e.getLongitude());
        double radius = exp(e.getMagnitude()) / 70000;
        //vec4 color = vec4(1,0,0,1);
	vec4 color = radius > 0.02? vec4(1,0,0,1) : vec4(0,1,0.5,1);
	//std::cout << radius << std::endl;
        quake = new Sphere(position, radius, color);
        quake->draw(_shader, model);
